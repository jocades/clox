...
static int constantInstruction(const char* name, Chunk* chunk, int offset) {
  ...
  printf("'\n");
  return offset + 2;  // opcode + operand (constant index)
}
...
int disassembleInstruction(Chunk* chunk, int offset) {
  ...
  switch (instruction) {
    ...
    case OP_GET_SUBSCRIPT: return simpleInstruction("OP_GET_SUBSCRIPT", offset);
    case OP_SET_SUBSCRIPT: return simpleInstruction("OP_SET_SUBSCRIPT", offset);
